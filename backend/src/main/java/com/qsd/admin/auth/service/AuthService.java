...
public class AuthService {
    ...
    public String login(String username, String password, String clientIp) {
        ...
        String rateLimitKey = "admin:" + tenant.getTenantCode() + ":"
            + (clientIp != null ? clientIp : "unknown") + ":" + username;
        if (!rateLimiterService.isAllowed(rateLimitKey)) {
            ...
            throw new BusinessException(ErrorCode.RATE_LIMITED,
                "Login attempts are too frequent, retry after " + remaining + " seconds");
        }
    ...
    public MeResponse me(AuthenticatedUser authenticatedUser) {
        ...
        Tenant loginTenant = sourceTenantId.equals(tenantId)
            ? tenant
            : tenantService.requireTenantById(sourceTenantId);
        List<String> permissions = adminUserMapper.selectPermissionCodes(user.getId());
