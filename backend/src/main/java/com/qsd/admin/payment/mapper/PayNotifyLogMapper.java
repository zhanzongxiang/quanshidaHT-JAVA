...
public interface PayNotifyLogMapper extends BaseMapper<PayNotifyLog> {
    ...
    @Select("""
        select id, tenant_id, pay_order_id, notify_type, resource_id, notify_status, raw_payload, process_result,
        ...
        order by id desc
        """)
    List<PayNotifyLog> selectByPayOrderId(@Param("tenantId") Long tenantId, @Param("payOrderId") Long payOrderId);
    ...
    @Select("""
        ...
        limit 1
        """)
    PayNotifyLog selectActiveById(@Param("tenantId") Long tenantId, @Param("id") Long id);

    @Select("""
        select notify_status as category, count(*) as count, max(created_at) as latestCreatedAt
        from pay_notify_log
        where tenant_id = #{tenantId}
          and notify_status not in ('paid', 'closed', 'SUCCESS', 'received')
